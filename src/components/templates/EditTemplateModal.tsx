...
export function EditTemplateModal({ template, onClose, onSuccess }: EditTemplateModalProps) {
  ...
  const [corPrimaria, setCorPrimaria] = useState(template.estrutura?.identidadeVisual?.corPrimaria || '#009B60')
  const [corSecundaria, setCorSecundaria] = useState(template.estrutura?.identidadeVisual?.corSecundaria || '#003333')
  const [capitulos, setCapitulos] = useState<string[]>(
    template.estrutura?.capitulos?.map((c: any) => c.nome) || []
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError('Erro ao atualizar template. Tente novamente.')
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">✏️ Editar Template</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={template.serie === 'PRIMEIRO_ANO' ? '1º Ano' : template.serie === 'SEGUNDO_ANO' ? '2º Ano' : template.serie === 'TERCEIRO_ANO' ? '3º Ano' : 'Cursinho'}
                disabled
